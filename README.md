...
import { EmailBuilder, BeefreeService } from '@beefree.io/angular-email-builder'
...
The library provides four standalone component selectors, all backed by the same `BeefreeBuilder` base class. The builder type is determined by the SDK configuration, not the selector — these are semantic aliases for template readability:

| Component | Selector | Usage |
|-----------|----------|-------|
| `BeefreeBuilder` | `lib-beefree-builder` | Generic builder |
| `EmailBuilder` | `lib-email-builder` | Email builder |
| `PageBuilder` | `lib-page-builder` | Page builder |
| `PopupBuilder` | `lib-popup-builder` | Popup builder |
| `FileManager` | `lib-filemanager` | File manager |
...
import { BeefreeService, EmailBuilder } from '@beefree.io/angular-email-builder'
...
```typescript
// DON'T DO THIS!
const token = await fetch('https://auth.getbee.io/apiauth', {
  method: 'POST',
  body: JSON.stringify({
...
### What is the difference between the builder components?

`EmailBuilder`, `PageBuilder`, `PopupBuilder`, and `FileManager` are all convenience aliases for the same `BeefreeBuilder` base component — they only differ in their selector name. The actual builder type is determined by the SDK configuration (specifically the application credentials), not by which component you use. The aliases exist for template readability.
